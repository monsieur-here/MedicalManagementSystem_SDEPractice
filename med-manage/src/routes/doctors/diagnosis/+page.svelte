...
<input type="text" inputmode="numeric" pattern="[0-9]*" placeholder="Appointment ID" bind:value={appointmentId} required />
<input type="text" inputmode="string" pattern="[a-zA-Z]*" placeholder="Diagnosis" bind:value={diagnosis} required />

	<h3>Prescribe Medication</h3>
	<div class="medication-form">
		<input type="text" placeholder="Medication Name(s)" bind:value={newMed.name} />
<input type="date" bind:value={newMed.dateIssued} min={new Date().toISOString().split('T')[0]} />		<textarea placeholder="Description" bind:value={newMed.description}></textarea>
	</div>
