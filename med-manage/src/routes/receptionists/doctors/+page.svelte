  ...
  onMount(() => {
    doctors.forEach((doc) => {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        availability[doc.doctorId][dateStr] = slots.filter(
          () => Math.random() > 0.3
        );
      }
    });
...
<div class="container">
  ...
  {#if selectedDoctor && selectedDate}
    <div class="slots">
      <h2>
        Available Slots for {doctors.find((d) => d.doctorId === +selectedDoctor)
          ?.doctorName} on {selectedDate}
      </h2>
      {#if availableSlots.length > 0}
        <ul>
          {#each availableSlots as time}
            <li>
              <button on:click={() => confirmAppointment(time)}>{time}</button>
            </li>
          {/each}
        </ul>
      {:else}
  ...
  {#if showConfirmation}
    <div class="popup">
      ✅ Appointment confirmed with {doctors.find(
        (d) => d.doctorId === +selectedDoctor
      )?.doctorName} on {selectedDate} at {selectedTime}
    </div>
  {/if}
</div>
